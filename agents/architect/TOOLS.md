...
**Option 1 — `gh` CLI** (recommandé) :
```bash
gh issue list --repo Liboudou/Trading_Bot
gh pr list --repo Liboudou/Trading_Bot
gh api repos/Liboudou/Trading_Bot --jq '.default_branch'
```

**Option 2 — MCP GitHub** (outil `mcp_github_*`) :
- Token configuré dans openclaw.json
- Utile pour : lire des fichiers, chercher du code, accéder aux contents
